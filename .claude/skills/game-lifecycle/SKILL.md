...
2. **KNI's `window` touch listeners suppress the synthesized `click`.** `Wasm.Dom/.../Window.8.0.10.js` adds `window` `touchstart`/`touchmove`/`touchend` listeners that call `event.preventDefault()` **unconditionally** for every touch on the page (active once any game runs). Chrome makes `window` `touchstart`/`touchmove` **passive**, so their `preventDefault` is *ignored* (overlay list scrolling still works) — but `touchend` is **not** passive, so its `preventDefault` **is** honored and kills the compatibility `click`. Result: overlay buttons highlight on tap (`:active` is a passive touchstart effect) but their `@onclick` never fires. **Fix:** stop touch from reaching KNI's `window` listeners — `@ontouchstart/move/end/cancel:stopPropagation="true"` on the overlay root. Blazor's delegated handler runs below `window` in the bubble path, so it strips the event before KNI. **Stop all four phases, not just touchend** — swallowing only the release while KNI still sees the press leaves a dangling `nativeTouchId` (the #90 bug-2 crash). This restores plain `@onclick` for the modal's close/card/category controls, so — unlike the toolbar — they need **no** `data-touch-action` (which would also swallow `touchmove` and kill list scrolling).
