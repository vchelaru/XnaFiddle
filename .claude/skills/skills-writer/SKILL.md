...
2. Check `TECHNICAL_PLAN.md` for existing coverage of the topic — including whether the finding is a specific instance of a general principle another skill already documents (see "Converging pull" above).
...
- Folder name must match `name` (kebab-case noun phrases, e.g. `game-lifecycle`, `shaders`).
...
- **Drop boilerplate.** No "Reference guide for…", no "Load this when working on…", no "Covers XnaFiddle's…". The fact that this is a skill is implicit.
...
❌ "Reference guide for XnaFiddle's shader compilation system. Load this when working on .fx files, ShadowDusk.Wasm, the SHADOWDUSK build gate, or Content.Load<Effect>."

✅ "XnaFiddle shader compilation via ShadowDusk. Triggers: .fx tabs, ShadowDusk.Wasm, SHADOWDUSK build gate, Content.Load<Effect>."
...
- Already in `TECHNICAL_PLAN.md` — link instead of restating.
