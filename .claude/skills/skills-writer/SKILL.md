...
A skill is **a map and a list of landmines**, not an encyclopedia. It points an agent at the right code and docs and warns about what isn't obvious from reading them. If a fact already lives in source or `TECHNICAL_PLAN.md`, **link, don't restate**.
...
Bad (states the same thing twice): *"Content loading doesn't have to go through TitleContainer's normal path — some assets are intercepted via an XHR shim instead. Grepping for a direct file read can therefore miss where an asset actually resolves, since it's only reachable through the intercept."*

Good (one pass, no restatement): *"TitleContainer.OpenStream is XHR-intercepted for embedded/user assets — check the intercept before assuming a direct file read."*
...
**Signpost quality bar.** A nudge must name *where to look* — a file, class, or relationship — not merely assert that something exists. "Compilation interacts with the render loop" raises a question without reducing search cost; "see `_pendingCompile` in `Index.razor.cs` — routes compilation through rAF to avoid a Monitor deadlock" reduces it. A vague signpost is worse than none: it costs context and resolves nothing.
...
- **`TECHNICAL_PLAN.md`** — comprehensive project documentation; link rather than restate.
- **Other skills** — cross-reference instead of copying.

## Process

1. Read the relevant source files.
2. Check `TECHNICAL_PLAN.md` for existing coverage of the topic — link it instead of restating.
...
- **Naming**: kebab-case noun phrases (e.g., `game-lifecycle`, `shaders`).
...
- **Drop boilerplate.** No "Reference guide for…", no "Load this when working on…", no "Covers XnaFiddle's…". The fact that this is a skill is implicit — these phrases are dead weight on every entry.
...
Good:
```
description: XnaFiddle shader compilation via ShadowDusk. Triggers: .fx tabs, ShadowDusk.Wasm, SHADOWDUSK build gate, Content.Load<Effect>.
```

Bad (boilerplate, padded):
```
description: Reference guide for XnaFiddle's shader compilation system. Load this when working on .fx files, ShadowDusk.Wasm, the SHADOWDUSK build gate, or Content.Load<Effect>.
```
...
- Pointers: links to `TECHNICAL_PLAN.md`, key source files, and related skills.
- Specific identifiers only when the name itself is misleading or the behavior is surprising.

## Exclude

- Anything already in `TECHNICAL_PLAN.md` — link instead of restating.
